...
module GitHubChangelogGenerator
  class Parser
    ...
    def self.parse_options
      ...
      parser.load
      parser.parse!
      parser.load(@@options.options) if @@options.options

      user_and_project_from_git
    ...
    def self.user_and_project_from_git
      detect_user_and_project unless @@options.user && @@options.project
      @@options.repo ||= "#{@@options.user}/#{@@options.project}"
    end

    # Detects user and project from git
    def self.detect_user_and_project(arg0 = nil, arg1 = nil)
      if @@options.repo
